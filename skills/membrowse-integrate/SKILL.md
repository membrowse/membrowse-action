...
- **Make-based projects**: Check if the Makefile already passes `-Wl,-Map` or has a `MAP` variable. If not, append the flag via a Make variable override, e.g., `make BOARD=PYBV10 LDFLAGS_EXTRA="-Wl,-Map=build/firmware.map"` — but check that the Makefile actually uses `LDFLAGS_EXTRA` or an equivalent. If the Makefile doesn't expose a way to add linker flags, the map file may need to be skipped for that target.
- **CMake projects**: Pass `-DCMAKE_EXE_LINKER_FLAGS="-Wl,-Map=${PWD}/build/output.map"` at configure time.
- **ESP-IDF projects**: The build system generates a map file automatically at `build/<project_name>.map`. No flag changes needed — just set the `map_file` field.
- **IAR projects**: Map files are generated by default. Check the project settings for the output path.
...
- Map file path (if build command generates one — enables library/object attribution)
...
- `map_file`: Path to the linker map file generated by the build command; empty string `""` if the build doesn't produce one. Must match the path used in the `-Wl,-Map=` flag in `build_cmd`. Supports GNU LD and IAR EWARM formats.
