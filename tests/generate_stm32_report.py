...
def generate_stm32_report(target_name='NUCLEO_F401RE_basic', output_file='stm32_memory_report.json'):
    ...
    try:
        ...
        if isinstance(validation_result, dict):
            ...
            validation_result = {'valid': bool(validation_result), 'warnings': []}
        ...
        report = {
            ...
            'statistics': {
                ...
                'flash_regions': len([r for r in parsed_regions.values() if r.get('type', '').upper() in ['FLASH', 'ROM']]),
                'ram_regions': len([r for r in parsed_regions.values() if r.get('type', '').upper() == 'RAM']),
                'total_flash_size': sum([r.get('limit_size', 0) for r in parsed_regions.values() if r.get('type', '').upper() in ['FLASH', 'ROM']]),
                'total_ram_size': sum([r.get('limit_size', 0) for r in parsed_regions.values() if r.get('type', '').upper() == 'RAM'])
            }
        }
        ...
        print(f"FLASH regions: {stats['flash_regions']} ({stats['total_flash_size']:,} bytes)")
        print(f"RAM regions: {stats['ram_regions']} ({stats['total_ram_size']:,} bytes)")
        ...
        for name, region in parsed_regions.items():
            ...
            print(f"{name:20} {region_type:8} 0x{addr:08x} - 0x{addr+size-1:08x} ({size:8,} bytes)")
...
if __name__ == '__main__':
    ...
    success = generate_stm32_report(target, output)
