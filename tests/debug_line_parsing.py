    ...
    try:
        with open(elf_file, 'rb') as f:
            ...
            for cu in dwarfinfo.iter_CUs():
                ...
                print(f"  ✅ Line program found")
                
                # Debug file and directory access
                try:
                    file_entries = lineprog['file_entry'] if 'file_entry' in lineprog else lineprog.header.file_entry
                    include_dirs = lineprog['include_directory'] if 'include_directory' in lineprog else lineprog.header.include_directory
                    ...
                    for i, file_entry in enumerate(file_entries):
                        ...
                        print(f"      File {i+1}: {filename} (dir_index: {getattr(file_entry, 'dir_index', 'N/A')})")
                    ...
                    for entry in lineprog.get_entries():
                        ...
                        # Check what we have
                        has_address = hasattr(state, 'address') and state.address is not None
                        has_file = hasattr(state, 'file') and state.file is not None and state.file > 0
                        ...
                        if not is_end_seq and has_address and has_file:
                            ...
                            if valid_entries <= 5:  # Show first 5
                                ...
                                if isinstance(filename, bytes):
                                    filename = filename.decode('utf-8', errors='ignore')
                                print(f"      Entry: addr=0x{state.address:x}, file={state.file} ({filename})")
                    
                    print(f"    Total entries: {entry_count}, Valid: {valid_entries}")
