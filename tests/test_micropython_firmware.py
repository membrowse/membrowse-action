...
class TestMicroPythonFirmware(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.firmware_path = Path("../micropython/ports/stm32/build-PYBV10/firmware.elf")
        cls.linker_script_path = Path("../micropython/ports/stm32/boards/stm32f405xg.ld")
        
        # Check if firmware exists
        if not cls.firmware_path.exists():
            raise unittest.SkipTest(f"MicroPython firmware not found at {cls.firmware_path}")
    ...
    def test_micropython_firmware_analysis(self):
        ...
        try:
            ...
            generator = MemoryReportGenerator(str(self.firmware_path), memory_regions_data)
            ...
            if uart_init_symbol:
                ...
                if uart_init_symbol['source_file']:
                    ...
                    print(f"📍 uart_init maps to: {uart_init_symbol['source_file']}")
                    
                    # This is what we discovered: in real firmware, functions may legitimately map to headers
                    # if that's where the line information points (e.g., inline functions or static functions in headers)
                    
                    # Should not be a system header though
                    self.assertNotIn('stdint', uart_init_symbol['source_file'].lower())
                    self.assertNotIn('stdio', uart_init_symbol['source_file'].lower()) 
                    self.assertNotIn('/usr/include', uart_init_symbol['source_file'])
                    
                    # Accept both .c and .h files - real firmware may have legitimate header mappings
                    if uart_init_symbol['source_file'].endswith('.c'):
                        print(f"✅ uart_init maps to .c source file: {uart_init_symbol['source_file']}")
                    elif uart_init_symbol['source_file'].endswith('.h'):
                        print(f"ℹ️  uart_init maps to .h header file: {uart_init_symbol['source_file']}")
                        print("    This may be legitimate for inline/static functions in headers")
                    else:
                        print(f"⚠️  uart_init maps to unexpected file type: {uart_init_symbol['source_file']}")
                    
                    # The key test: it should not be a system/standard library header
                    is_project_file = (
                    ...
                    self.assertTrue(is_project_file, 
                                  f"uart_init should map to project file, not system header: {uart_init_symbol['source_file']}")
                    
                    print(f"✅ uart_init correctly maps to project file: {uart_init_symbol['source_file']}")
                else:
                    print("⚠️  uart_init has empty source_file - this might be expected for optimized builds")
                ...
                if uart_funcs:
                    ...
                    if test_func['source_file']:
                        ...
                        print(f"✅ {test_func['name']} correctly maps to: {test_func['source_file']}")
            ...
            if i2c_handle1_symbol:
                ...
                if i2c_handle1_symbol['source_file']:
                    if i2c_handle1_symbol['source_file'].endswith('.h'):
                        print(f"  ⚠️  I2CHandle1 maps to header file: {i2c_handle1_symbol['source_file']}")
                        print(f"      Should be defined in pyb_i2c.c or similar .c file")
                    else:
                        print(f"  ✅ I2CHandle1 correctly maps to: {i2c_handle1_symbol['source_file']}")
            else:
                print(f"\n⚠️  I2CHandle1 not found")
                print(f"Found {len(i2c_symbols)} I2C-related symbols:")
                for sym in i2c_symbols[:10]:
                    print(f"  - {sym['name']}: {sym.get('source_file', 'no source')}")
            
            # Print summary statistics
            total_symbols = len(report['symbols'])
            symbols_with_source = len([s for s in report['symbols'] if s['source_file']])
            symbols_without_source = [s for s in report['symbols'] if not s['source_file']]
            
            print(f"\nReport summary:")
            print(f"  Total symbols: {total_symbols}")
            print(f"  Symbols with source files: {symbols_with_source}")
            print(f"  Symbols without source files: {len(symbols_without_source)}")
            ...
            by_type = {}
            by_section = {}
            by_name_pattern = {'compiler_generated': [], 'asm_related': [], 'lib_related': [], 'other': []}
            ...
            for symbol in symbols_without_source:
                ...
                if any(pattern in name for pattern in ['__', '_start', '_end', '_size', 'thunk', 'trampoline', 'stub']):
                    by_name_pattern['compiler_generated'].append(symbol['name'])
            ...
            for sym_type, count in sorted(by_type.items(), key=lambda x: x[1], reverse=True):
            ...
            for i, symbol in enumerate(symbols_without_source[:20]):
                print(f"  {i+1:2d}. {symbol['name']} (type={symbol['type']}, section={symbol['section']}, addr=0x{symbol['address']:08x}, size={symbol['size']})")
            ...
            symbols_with_source_dict = {s['address']: s for s in report['symbols'] if s['source_file']}
            
            # First, let's check which compilation units these symbols belong to
            ...
            for symbol in symbols_without_source[:10]:  # Check first 10
                ...
                for addr, nearby_symbol in symbols_with_source_dict.items():
                    if abs(addr - symbol['address']) <= 200:  # Within 200 bytes
                        nearby_with_source.append((abs(addr - symbol['address']), nearby_symbol))
                ...
                if nearby_with_source:
                    ...
                    print(f"    Closest symbol with source: {closest['name']} ({closest['source_file']}) at distance {distance} bytes")
                    
                    # If very close to a symbol with source file, this might be a missing mapping
                    if distance <= 50:
            ...
            if suspicious_symbols:
                ...
                for symbol, closest, distance in suspicious_symbols:
                    print(f"  - {symbol['name']} should probably map to {closest['source_file']} (distance: {distance} bytes)")
            else:
